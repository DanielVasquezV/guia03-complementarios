...
const ProductItem = ({ producto, onAgregar }: ProductItemProps): React.JSX.Element => (
    <div style={{
        ...
        <h4 style={{ margin: '0 0 6px', color: '#001f3f', fontSize: '.9rem' }}>{producto.nombre}</h4>
        ...
        <button
            ...
            style={{
                ...
                border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '.85rem'
            }}
        >
            + Agregar
        </button>
...
const CartSummary = ({ carrito, onEliminar, onLimpiar }: CartSummaryProps): React.JSX.Element => {
        ...
        <div>
            ...
            {carrito.map((item: ItemCarrito) => (
                <div key={item.id} style={{
                    ...
                    <div style={{ flex: 1 }}>
                        <p style={{ margin: 0, fontWeight: 600, fontSize: '.88rem', color: '#1a2332' }}>{item.nombre}</p>
                        <p style={{ margin: 0, fontSize: '.78rem', color: '#5a6a7e' }}>
                            ${item.precio} × {item.cantidad} = <strong>${(item.precio * item.cantidad).toLocaleString()}</strong>
                        </p>
                    </div>
                    <button onClick={() => onEliminar(item.id)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#c0392b', fontSize: '1.1rem' }}>
                        ✕
                    </button>
                </div>
            ))}

            <div style={{ marginTop: '16px', padding: '14px', background: '#e6f1fb', borderRadius: '8px' }}>
...
export default function ShoppingCart(): React.JSX.Element {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '28px', alignItems: 'start' }}>
            ...
            {/* Carrito */}
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 10px rgba(0,63,127,.08)' }}>
                <CartSummary
                    carrito={carrito}
