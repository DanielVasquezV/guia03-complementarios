...
type Campo = "nombre" | "email" | "password" | "confirmar" | "telefono";
...
const initialData: FormData = { nombre: "", email: "", password: "", confirmar: "", telefono: "" };
const initialErrors: FormErrors = { nombre: "", email: "", password: "", confirmar: "", telefono: "" };
...
function validarCampo(campo: Campo, valor: string, formData: FormData): string {
    switch (campo) {
        ...
        case "email":
            ...
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(valor)) return "Ingresa un email válido";
            return "";
        case "password":
        ...
        case "telefono":
            ...
            if (valor.length < 9) return "Mínimo 9 números";
            if (!/[0-9]/.test(valor)) return "Debe incluir al menos un número";
            return "";
        default:
            return "";
...
export default function RegisterForm(): React.JSX.Element {
    ...
    const handleSubmit = (): void => {
        const nuevosErrores: FormErrors = {
            ...
            telefono: validarCampo("telefono", formData.telefono, formData),
        };
        ...
        return (
            <div style={{ background: '#edfaf3', border: '2px solid #00a651', borderRadius: '12px', padding: '32px', textAlign: 'center' }}>
                ...
                <button onClick={() => { setFormData(initialData); setErrores(initialErrors); setExito(false); }}
                    style={{ marginTop: '16px', padding: '8px 24px', background: '#003f7f', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>
                    Nuevo registro
                </button>
            </div>
        );
    ...
    return (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '32px', maxWidth: '460px', boxShadow: '0 2px 12px rgba(0,63,127,.08)' }}>
            <h2 style={{ color: '#001f3f', marginBottom: '24px' }}>Crear cuenta</h2>

            {/* Campo Nombre */}
            <div style={{ marginBottom: '18px' }}>
                <label style={{ fontWeight: 600, color: '#1a2332', fontSize: '.9rem' }}>Nombre completo</label>
                <input
                    type="text"
                ...
                />
                {errores.nombre && <p style={{ color: '#c0392b', fontSize: '.78rem', margin: '4px 0 0' }}>⚠ {errores.nombre}</p>}
            </div>
            ...
            <div style={{ marginBottom: '18px' }}>
                ...
                />
                {errores.email && <p style={{ color: '#c0392b', fontSize: '.78rem', margin: '4px 0 0' }}>⚠ {errores.email}</p>}
            </div>

            {/* Campo Telefono */}
            <div style={{ marginBottom: '18px' }}>
                <label style={{ fontWeight: 600, color: '#1a2332', fontSize: '.9rem' }}>Telefono</label>
                <input
                    type="number"
                    value={formData.telefono}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange("telefono", e.target.value)}
                    placeholder="123456789"
                    style={inputStyle("telefono")}
                />
                {errores.telefono && <p style={{ color: '#c0392b', fontSize: '.78rem', margin: '4px 0 0' }}>⚠ {errores.telefono}</p>}
            </div>

            {/* Campo Password */}
            <div style={{ marginBottom: '18px' }}>
                <label style={{ fontWeight: 600, color: '#1a2332', fontSize: '.9rem' }}>Contraseña</label>
                <input
                    type="password"
                ...
                />
                {errores.password && <p style={{ color: '#c0392b', fontSize: '.78rem', margin: '4px 0 0' }}>⚠ {errores.password}</p>}
            </div>

            {/* Confirmar Password */}
            <div style={{ marginBottom: '24px' }}>
                <label style={{ fontWeight: 600, color: '#1a2332', fontSize: '.9rem' }}>Confirmar contraseña</label>
                <input
                    type="password"
            ...
            <button
                ...
                style={{ width: '100%', padding: '12px', background: '#003f7f', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '1rem', fontWeight: 700, cursor: 'pointer' }}
            >
                Registrarse
            </button>
        </div>
